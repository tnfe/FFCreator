...
class FFCreator extends FFCon {
  // 暴露出来是给pixi的plugin
  static InkPaint = require('inkpaint');
  ...
  async timeUpdate(delta = 0, timeInMs = -1) {
    ...
    this.time = time;
    // console.log('creator.timeUpdate', {ttt: this.time, timeInMs, delta, cbs:this.frameCallbacks.length});
  ...
  allNodes() {
    ...
    nodes = nodes.concat(this.audios);
    return nodes;
  }
