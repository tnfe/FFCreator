...
class FFCreator extends FFCon {
  constructor(conf = {}) {
    ...
    const { transformers, ...rest } = conf;

    this.audios = [];
    this.inCenter = false;
    this.conf = new Conf(rest);
    this.loader = new Loader();

    this.createApp();
    this.createRenderer({ transformers });
   ...
   * @private
   * @param {{
   *   transformers?: import("./core/renderer").RenderTransformer[];
   * }} props
   */
  createRenderer(props = {}) {
    this.renderer = new Renderer({ creator: this, ...props });
  }
