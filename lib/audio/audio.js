...
class FFAudio extends FFBase {
  ...
  preProcessing() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.volume > 0) this.$audio.volume = Math.min(this.volume, 1.0); // 暂时不能变大
    });
  }
  ...
  async grantPlay() {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        media.pause(); // 开始play之后，立刻暂停并恢复原先的mute状态
        media.muted = false;
  ...
  play(time) {
    ...
    if (this.playing) {
      this.playerDelay = seekTime - this.$audio.currentTime;
      return;
    }
    this.$audio.currentTime = seekTime;
    this.playing = true;
    this.$audio.playbackRate = this.material.creator.playbackRate;
    this.$audio.play();
  }
  ...
  async drawing(timeInMs = 0, nextDeltaInMS = 0) {
    ...
    nextTime < matDuration ? this.play(this.currentTime) : this.end(this.currentTime);
  }
