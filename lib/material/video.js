...
class VideoMaterial extends Material {
  ...
  ffCmdExec({ command, output, inOpts, outOpts, onProgress }) {
    return new Promise((resolve, reject) => {
      command.addInput(this.path);
      command.inputOptions(typeof(inOpts)=='string' ? inOpts.split(' ') : inOpts);
      command.outputOptions(typeof(outOpts)=='string' ? outOpts.split(' ') : outOpts);
