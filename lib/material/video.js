...
class VideoMaterial extends ImageMaterial {
  ...
  async init(opts) {
    ...
    if (isBrowser) {
      ...
      this.vCap = cv.VideoCapture(this.path);
  ...
  async getFrameByTime(time) {
    ...
    if (!isBrowser) {
      if (!this.vCap) return this.getFrame((time * this.fps) >> 0);
      return this.cutout(cv.getFrameByTime(this.vCap, seekTime));
    }
    return new Promise((resolve, reject) => {
  ...
  async extract(dir, name) {
    if (!this.vCap) await this.extractVideo(dir, `${name}_video`);
