...
class Renderer extends FFBase {
  ...
  async play(playRate=1.0) {
    ...
    this.emit({ type: 'playing', currentTime: this.tweenGroup.now() });
    this.playing = true;
    const ticker = 40;
    let aa = 0;
    const render = async () => {
      let adjust = this.main_delay();
      const err = adjust;
      if (err !== 0) {
        aa += adjust;
      ...
      if (this.playRate > 0) { // play
        this.timeline.nextFrame(ticker * playRate);
  ...
  async start() {
    ...
    try {
      ...
      if (isBrowser) throw error; //todo: 先这样，避免浏览器卡死
    }
  ...
  main_delay() {
    const creator = this.getCreator();
    if (!this.mPLAYER || !this.mPLAYER.player().playing) {
      this.mPLAYER = this.player(creator.mVIDEOS);
    }
    return this.mPLAYER ? (this.mPLAYER.delay() * 1000) >> 0 : 0;
  }
  ...
  createTimeline() {
    ...
    const creator = this.getCreator();
    const timeline = new Timeline(creator);
