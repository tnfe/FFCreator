...
class Renderer extends FFBase {
  ...
  async play(playRate=1.0) {
    ...
    const render = async () => {
      ...
      if (this.playRate > 0) { // play
        ...
        this.timeline.nextFrame(Math.max(1, ticker - adjust) * playRate);
  ...
  async start() {
    ...
    try {
      ...
      if (isBrowser) console.error(error); // throw error; todo: 先这样，避免浏览器卡死
    ...
    creator.audios.map(audio => audio.start());
  ...
  async preProcessing() {
    ...
    this.emit({ type: 'preloading', loaded, total });
    await awaitMap(nodes, async (node) => {
      ...
      while (true) {
        try {
          ...
          if (++i > (node.retry || 1)) throw `preprocess fail: ${node.id}`;
