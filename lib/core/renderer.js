...
class Renderer extends FFBase {
  ...
  async start() {
    ...
    // need timeline created
    if (this.backendRender) {
      this.createSynthesis();
      this.playRate = 1;
    } else {
      // front-end render: do not start play
      this.playRate = 0;
    }
  ...
  async renderFrame(callback) {
    ...
    FFLogger.info({ pos: 'renderFrame', msg: `current frame - ${timeline.frame}  play rate: ${this.playRate}` });
  ...
  createSynthesis() {
    ...
    console.log('xxxx sss', Synthesis);
    const synthesis = new Synthesis(conf);
    console.log('xxxx ooo', {synthesis, timeline});
