...
class Renderer extends FFBase {
  ...
  async jumpTo(timeInMs) {
    ...
    this.renderFrame();
    this.emit({ type: 'seeked', currentTime: TWEEN.now() });
  }
  ...
  async play(playRate=1.0) {
    ...
    this.emit({ type: 'playing', currentTime: TWEEN.now() });
    this.playing = true;
    const ticker = 40;
    let aa = 0;
    const render = async () => {
      let adjust = Material.MAIN_DELAY();
      const err = adjust;
      if (err !== 0) {
        aa += adjust;
      ...
      this.emit({ type: 'timeupdate', currentTime: TWEEN.now() });
      ...
      if (this.playRate > 0) { // play
        ...
        const now = TWEEN.now();
        if (now > this.timeline.duration * 1000) {
          this.emit({ type: 'ended' });
        } else {
          this.emit({ type: 'pause', currentTime: now });
        }
        this.renderFrame();
      }
    }
