...
class Synthesis extends FFEventer {
  ...
  addAudios(audios) {
    ...
    forEach(audios, audio => {
      command.addInput(audio.path);
      if (audio.loop) command.inputOptions(['-stream_loop', '-1']);
      this.inputCount++;
    });
    this.audios = audios;
  }
  ...
  addAudioFilter() {
    ...
    forEach(audios, (audio, index) => {
      ...
      const audioCommand = audio.toCommand({ index, duration });
      filters.push(audioCommand);
      outputs += `[${output}]`;
    });

    filters.push(`${outputs}amix=${audios.length}`);
