...
const requireTransition = require('gl-transition')
const createTransition = requireTransition.default ? requireTransition.default : requireTransition;
...
class FFTransition extends FFBase {
   ...
   * @private
   */
  async render({dir, from, to, progress}) {
    if (!from || !to) return;
    ...
    const fromPixels = await this.getPixels({dir, name: from, width, height});
    const toPixels = await this.getPixels({dir, name: to, width, height});
   ...
   * @private
   */
  async getPixels({dir, name, width, height}) {
