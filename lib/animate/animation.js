...
class FFAnimation extends FFBase {
  ...
  startTime() {
    ...
    // startTime:  this.parent.parent => scene
    return this._startTime = (this.parent?.parent?.startTime * 1000 + this.delay) >> 0;
  }
   ...
   */
  mayStart() {
    // console.log(this.parent.id, 'ani.mayStart', TWEEN.now(), '>', this.startTime());
  ...
  createTween() {
    ...
    return new TWEEN.Tween(from, this.tweenGroup)
      .to(to, this.time)
      .delay(this.delay)
      .easing(ease)
      .onStart(() => {
