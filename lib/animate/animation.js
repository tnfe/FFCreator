...
class FFAnimation extends FFBase {
  constructor(conf = { time: 0.7, delay: 0, ease: 'Linear.None' }) {
    ...
    this.from = from;
    // WTF!! FFAnimations.getDurationTime will use to.type=='out' to detect
    this.to = to;
    this.to.type = type;
    this.ease = ease;
    this.parent = parent;
  ...
  start() {
    ...
    this.tween.start();
    // 这里是强制设置它的开始时间(ms)，实现seek的需求 (this.parent.parent => scene)
    this.tween._startTime = (this.parent.parent.startTime * 1000 + this.delay) >> 0;
    console.log('ani.start', this.tween);
  }
