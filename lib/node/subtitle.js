...
class FFSubtitle extends FFNode {
  ...
  showNewTextLine(text) {
    ...
    switch (this.conf.animate) {
      case 'up':
        this.display.attr({text, opacity: 0});
        FFTween.spriteTweenFromTo(this.display, time, {opacity: 0, y: y + DIS}, {opacity: 1, y});
        break;
      ...
      case 'left':
        this.display.attr({text, opacity: 0});
        FFTween.spriteTweenFromTo(this.display, time, {opacity: 0, x: x - DIS}, {opacity: 1, x});
        break;
  ...
  destroy() {
    ...
    Timeline.removeFrameCallback(this.drawing);
