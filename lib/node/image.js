...
const { Sprite, Texture, Rectangle, createCanvas } = require('inkpaint');
...
class FFImage extends FFNode {
  ...
  fitTexture(src) {
    ...
    if (!width || !height || (width >= src.width && height >= src.height)) {
      src.texture.frame = new Rectangle(0, 0, src.texture.width, src.texture.height);
      return;
    }
    // clone是为了不影响loader里url作为key的公共素材
  ...
  draw({ display, texture, useCache = false }) {
    ...
    if (isBrowser && !(texture instanceof Texture)) {
      // do nothing...
