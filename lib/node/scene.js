...
class FFScene extends FFCon {
  ...
  //   forEach(this.audios, audio => audio.start());
  // }
  ...
  //   forEach(this.audios, audio => audio.end());
  // }
  ...
  annotate() {
    const maxAbsEnd = Math.max(...this.allNodes.map(x => x.absEnd));
    this.defaultDuration = maxAbsEnd - this.start;
  }

  get default() {
    return {
      start: super.default.start,
