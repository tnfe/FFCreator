...
class FFVideo extends FFImage {
  constructor(conf) {
    ...
    this.voImageExtra = conf.voImageExtra;
  ...
  getVOutput() {
    ...
    FS.ensureDir(dir);
    return path.join(dir, `${this.id}_%d.${this.getVOutputExt()}`);
  }

  getVOutputExt() {
    if (this.voImageExtra) return this.voImageExtra
    // ffprobe -v 0 -show_entries pixel_format=name:flags=alpha -of compact=p=0
    const pixelFormatsWithAlphaChannels = [
      "pal8", "argb", "rgba", "abgr", "bgra", "yuva420p", "ya8", "yuva422p",
      ...
      "yuva444p16be", "yuva444p16le", "rgba64be", "rgba64le", "bgra64be",
      "bgra64le", "ya16be", "ya16le", "gbrap", "gbrap16be", "gbrap16le",
      "ayuv64le", "ayuv64be", "gbrap12be", "gbrap12le", "gbrap10be",
      "gbrap10le", "gbrapf32be", "gbrapf32le", "yuva422p12be", "yuva422p12le",
      "yuva444p12be", "yuva444p12le"
    ];
    const hasAlphaChannel = this.materials.info['streams'].find((stream) => {
      return pixelFormatsWithAlphaChannels.includes(stream['pix_fmt'])
    }) !== undefined
